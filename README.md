...
- **ingest** — 신규 로그/보고 1건 → 관련 위키 페이지 갱신·크로스링크 (저장→질의 가능 ~4초)
- **query** — 오프라인 질의응답. [교범 조항 ID] + [보고 타임스탬프] 이중 인용 강제, 근거 없으면 거부, RBAC 프리필터
- **lint** — 모순·스테일 정보 자동 플래그 = 정보융합(intel fusion) 자동화
